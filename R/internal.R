...
#' @keywords internal
three.cal <- function(alpha1, alpha2, beta, pc, pt, n, sf = "Pocock", ...){
  # initialization
  nc <- cumsum(n)
  nt <- nc[3]
  if(sf == "Pocock" | sf == "OF") {
...
#' @keywords internal
two.cal <- function(alpha1, alpha2, beta, pc, pt, n, sf = "Pocock", ...){
  # initialization
  nc <- cumsum(n)
  nt <- nc[2]
  if(sf == "Pocock" | sf == "OF") {
  ...
  out_r2 <- out_r2[max(ind_r2)]
  s_bdry <- r2:nc[2]
  out_s <- sapply(s_bdry, function(ss) sum(dbinom(t1, n[1], pc[1]) * (1 - pbinom(ss-t1, n[2], pc[1]))))
  ind_s <-  which(out_s <= alpha2)
  # check if conditions hold
  if(length(ind_s) == 0) next
  s <- s_bdry[min(ind_s)]
  out_s <- out_s[min(ind_s)]
  # calculate type 2 error 
  beta_t <- sum(dbinom(t1, n[1], pt) * pbinom(s-t1, n[2], pt))				
  ind_beta <- which(beta_t <= beta)
  if(length(ind_beta) == 0) return(NULL)
  # save feasible designs & errors
  out <- c(r1, r2, s, out_r1, out_r2, out_s, beta_t)
  return(out)
}

#' @keywords internal
opt.three <- function(alpha1, alpha2, beta, pc, pt, frac_n1 = c(0.2, 0.3), frac_n2 = c(0.2,0.4), sf = "Pocock", show = FALSE,...) {
  if(length(pc) == 1) {
    pc <- rep(pc, 2)
  }
  nt <- 3
  ...
  while(is.null(out)) {
    ...
    for(i in 1:nrow(n)){
      ans <- three.cal(alpha1, alpha2, beta, pc, pt, n[i, ])
  ...
  return(list(bdry = bdry, error = error, n = nn))
}

#' @keywords internal
opt.two <- function(alpha1, alpha2, beta, pc, pt,  sf = "Pocock", show = FALSE, ...) {
  if(length(pc) == 1) {
    pc <- rep(pc, 2)
  }
  nt <- 2
  ...
  while(is.null(out)) {
    for(n1 in 1:(nt-1)) {
      n2 <- nt-n1
      ans <- two.cal(alpha1, alpha2, beta, pc, pt, c(n1, n2))
      if(!is.null(ans)) comb.n <- rbind(comb.n, c(n1, n2))
      out <- rbind(out, ans)
    }
    nt <- nt+1
    if(show) print(paste("current sample size is", nt))
  ...
  out <- cbind(out, comb.n)
  out <- out[order(-out[,5], -out[, 6], out[,7]), ]
  if(is.matrix(out)) out <- out[1,]
  names(out) <- c("r1", "r2","s", "alpha11", "alpha12", "alpha2", "beta", "n1", "n2")
  return(list(bdry = out[1:3], error = out[4:7], n = out[8:9]))
}