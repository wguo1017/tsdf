...
two.opt <- function(alpha1, alpha2, pc, n, sf = "Pocock", ...){
  ...
  for(r1i in 1:length(r1)){
    ...
    for(s1i in 1:length(s1)){
      ...
      for(r2i in 1:length(r2)){
        ...
        comb <- rbind(comb, cbind(r1[r1i], r2[r2i], s1[s1i], s2))
        err <- rbind(err, cbind(out_r1[r1i], out_r2[r2i], out_s1[s1i], out_s2))
      }
    }
  }
  # merge results
  if(is.null(comb)){
    ...
    rs <- c("r1", "r2", "s1", "s2")
    err1 <- c("alpha11", "alpha12", "alpha21", "alpha22")
    out <- cbind(comb, err)
    out <- out[order(-out[,5], -out[,7], -out[,6], -out[,8]), ]
    colnames(out) <- c(rs, err1)
  }
...
right.two.opt <- function(alpha, pc, n, sf = "Pocock", ...){
  ...
  for(s1i in 1:length(s1)){
    ...
    comb <- rbind(comb, cbind(s1[s1i], s2))
    err <- rbind(err, cbind(out_s1[s1i], out_s2))
  }
  # merge results
  if(is.null(comb)){
    return(print("No optimal design (right side)"))
    ...
    rs <- c("s1", "s2")
    err1 <- c("alpha11", "alpha12")
    out <- cbind(comb, err)
    out <- out[order(-out[,3], -out[,4]), ]
    colnames(out) <- c(rs, err1)
  }
...
right.three.opt <- function(alpha, pc, n, sf = "Pocock", ...){
  ...
  if(is.null(comb)){
    ...
    err2 <- c("beta")
    out <- cbind(comb, err)
    out <- out[order(-out[,4], -out[,5], -out[,6], out[,7]), ]
    colnames(out) <- c(rs, err1, err2)
  }
  out <- list(out = out, pc = pc, n = n, sf = sf, alpha = alpha)
  class(out) <- "1opt"
